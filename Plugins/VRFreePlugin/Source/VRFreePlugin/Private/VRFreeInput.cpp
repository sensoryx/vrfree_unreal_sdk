	...
	: MessageHandler(InMessageHandler)
{
	...
	int error = VRfree_Init(DotNetPathPtr);
	if (error != 0) {
...
void FVRFreeInput::TickCalibration()
{
	...
	if (LeftRapidMovementCounter == 0 && RightRapidMovementCounter == 0 && LeftActivityCounter > 35 && RightActivityCounter > 35)
	{
		...
		if (calibrationCounter > 1.0f)
		{
			...
			VRfree_calibratePose(false, LeftCalibrationPosePhase1);
			VRfree_calibratePose(true, RightCalibrationPosePhase1);
			operatingMode = EVRFreePluginMode::RUNNING;
			reportingLiveData = true;
		}
	}
...
//	UE_LOG(VRFreeInputLog, Log, TEXT("VRFree tick"));
	...
	VRfree_updateCameraPose(VectorToVRFree(HMDPos), QuatToVRFree(FQuat(HMDRot)), false);

	currentDataLeft.Load(VRfree_getHandData(false));
	currentDataRight.Load(VRfree_getHandData(true));
	...
	VRfree_getTrackerData(trackerPosition, trackerRotation, isTrackerPositionValid, buttonPressed, trackerId);
